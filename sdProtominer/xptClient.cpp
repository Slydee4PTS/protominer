...
xptClient_t* xptClient_connect(generalRequestTarget_t* target, uint32 payloadNum)
{
    ...
    // change SD
	//unsigned int cbRet;
    //SAIoctl(clientSocket, FIONBIO, &nonblocking, sizeof(nonblocking), NULL, 0, (LPDWORD)&cbRet, NULL, NULL);
    
    ioctl(clientSocket, FIONBIO, &nonblocking);
...
//	InitializeCriticalSection(&xptClient->cs_workAccess);
    ...
    pthread_mutex_init(&xptClient->mutex_shareSubmit, NULL);
    pthread_mutex_init(&xptClient->mutex_workAccess, NULL);
...
void xptClient_free(xptClient_t* xptClient)
{
	...
	{
		shutdown(xptClient->clientSocket, 0);
	}
...
{
	//uint32 usernameLength = min(127, (int)strlen(xptClient->username));
	//uint32 passwordLength = min(127, (int)strlen(xptClient->password));
	...
	xptPacketbuffer_writeString(xptClient->sendBuffer, (char *)minerVersionString, 45, &sendError);		// minerVersionString
...
bool xptClient_process(xptClient_t* xptClient)
{
    ...
    pthread_mutex_lock(&xptClient->mutex_shareSubmit);
    ...
    pthread_mutex_unlock(&xptClient->mutex_shareSubmit);
