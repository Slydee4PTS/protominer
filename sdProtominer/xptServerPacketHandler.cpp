...
bool xptServer_sendAuthResponse(xptServer_t* xptServer, xptServerClient_t* xptServerClient, uint32 authErrorCode, char* rejectReason)
{
    ...
    size_t rejectReasonLength = strlen(rejectReason);
    ...
    // send to client
	send(xptServerClient->clientSocket, (const void*)(xptServer->sendBuffer->buffer), xptServer->sendBuffer->parserIndex, 0);
