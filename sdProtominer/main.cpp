...
const char* minerVersionString = "sdProtominer v0.1";
const char* default_server = "ypool.net";
...
pthread_mutex_t mutex_xptClient = PTHREAD_MUTEX_INITIALIZER;
...
void jhProtominer_submitShare(minerProtosharesBlock_t* block)
{
    ...
    pthread_mutex_lock(&mutex_xptClient);
    
	if( xptClient == NULL )
	{
		printf("Share submission failed - No connection to server\n");
        pthread_mutex_unlock(&mutex_xptClient);
		return;
	}
...
void* jhProtominer_minerThread(void* data)
{
	while( true )
	{
		// local work data
        ...
        pthread_mutex_lock(&workDataSource.mutex_work);
        ...
        pthread_mutex_unlock(&workDataSource.mutex_work);
...
void jhProtominer_xptQueryWorkLoop()
{
    ...
    long timerPrintDetails = GetTickCount() + 8000;
	...
	while( true )
	{
		...
		if( currentTick >= timerPrintDetails )
		{
			...
			if( xptClient )
			{
				long passedSeconds = time(NULL) - miningStartTime;
		...
		if( xptClient )
		{
			...
			//	LeaveCriticalSection(&workDataSource.cs_work);
				...
				// we lost connection :(
				printf("Connection to server lost - Reconnect in 10 seconds\n");
		...
		else
		{
			...
			//	LeaveCriticalSection(&cs_xptClient);
				...
				printf("Connection attempt failed, retry in 10 seconds\n");
				sleep(10000);
			}
			else
			{
	...
	while( cIdx < argc )
	{
		...
		if( memcmp(argument, "-o", 3)==0 || memcmp(argument, "-O", 3)==0 )
		{
			...
			if( strstr(argv[cIdx], "http://") )
				commandlineInput.host = strdup(strstr(argv[cIdx], "http://")+7);
			else
				commandlineInput.host = strdup(argv[cIdx]);
		...
		else if( memcmp(argument, "-u", 3)==0 )
		{
			...
			commandlineInput.workername = strdup(argv[cIdx]);
			cIdx++;
		}
		else if( memcmp(argument, "-p", 3)==0 )
		{
			...
			commandlineInput.workerpass = strdup(argv[cIdx]);
			cIdx++;
		}
...
int main(int argc, char** argv)
{
    ...
    pthread_mutex_init(&mutex_xptClient, NULL);
    pthread_mutex_init(&workDataSource.mutex_work, NULL);
	...
	for(uint32 i=0; i<commandlineInput.numThreads; i++)
    {
        ...
        pthread_create(&posixThreadID, NULL, &jhProtominer_minerThread, NULL);
    }
